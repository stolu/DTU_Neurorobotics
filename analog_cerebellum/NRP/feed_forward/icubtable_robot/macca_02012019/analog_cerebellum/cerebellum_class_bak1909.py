...
class Cerebellum:
    ...
    def __init__(self,n_uml, n_out, n_input_mossy, n_input_pf):
        ...
        self.output_pc_dcn    = [0 for k in range(self.n_out_pf)]
        self.IO_fbacktorq   = []
    ...
    def prediction(self, fbacktorq, pos_error, vel_error, iodcn_signal):   
        ...
        for i in range(0,self.n_uml):
            ...
            if self.tau_normalization == 1:                
                self.IO_fbacktorq[i] = ( self.norma( abs(fbacktorq[i]) , self.min_teach_pos[i], self.max_teach_pos[i] , self.tau_norm_sign) + self.IO_fbacktorq_t[i] ) /2.
...
#            self.w_pf_pc[i]  = self.w_pf_pct[i]  + self.ltpPF_PC_max /( self.IO_fbacktorq[i] + 1.)**(self.alphaPF_PC) - self.ltdPF_PC_max*self.IO_fbacktorq[i] 
            ...
            if self.debug_prediction == 1:
                ...
                print(" \n self.w_pf_pc_vel[i]  joint "+str(i)+" "+str( self.w_pf_pc_vel[i]  ))
            
            # output signal PC
            self.output_pc[i]    = self.w_pf_pc[i]*self.output_x_pf[i] + self.w_pf_pc_vel[i]*self.output_x_pf[i] #*np.sum(self.weights_mod_pf)
            if self.debug_prediction == 1:
            ...
            if self.tau_normalization == 1: 
                self.output_pc_dcn[i] = self.norma( abs( self.output_pc[i] ) , 0. , 1.5 , self.tau_norm_sign)
            else:
                self.output_pc_dcn[i] = abs(self.output_pc[i])
                
            self.w_pc_dcn[i]  = self.w_pc_dcnt[i] + ( self.ltpPC_DCN_max*(self.output_pc_dcn[i]**self.alphaPC_DCN) )/( 1. + self.norma( abs( self.output_DCN[i] ) , 0. , 1.5 , self.tau_norm_sign) )**self.alphaPC_DCN - self.ltdPC_DCN_max*( self.output_pc_dcn[i]  )#1. - self.output_pc[i])
            ...
            self.w_mf_dcn[i]  = self.w_mf_dcnt[i] + self.ltpMF_DCN_max/( self.output_pc_dcn[i]  + 1.)**self.alphaMF_DCN - self.ltdMF_DCN_max*self.output_pc_dcn[i] 
            ...
            # IO - DCN 
            self.w_io_dcn[i] = self.w_io_dcnt[i] + self.ltpIO_DCN_max*iodcn_signal[i] - self.ltdIO_DCN_max/( (iodcn_signal[i] + 1.)**self.alphaIO_DCN  )    
            self.w_io_dcnt[i] = self.w_io_dcn[i]
            
            self.output_IO_DCN[i] =  self.w_io_dcn[i]*iodcn_signal[i]
            
            if self.debug_prediction == 1:
                print("\n self.w_io_dcn[i] "+str( self.w_io_dcn[i] ))
                print("\n iodcn_signal[i] "+str( iodcn_signal[i] ))
                print("\n self.output_IO_DCN[i] "+str( self.output_IO_DCN[i] ))
            # DCN out            
            self.output_DCN[i] =  - self.w_pc_dcn[i]*self.output_pc[i] + self.output_C_mossy[i] #self.output_x_mossy[i][0]  #+ self.output_x_mossy[i][0]
            if self.debug_prediction == 1:
