...
class MLandC:
    ...
    def __init__(self, nin, nout, njoints):
        ...
        for i in range(nout):
            ...
            self.w[i] = np.array([0])
        self.model = LWPR(self.nin, self.nout)  # nout = 1
    ...
    # inputs to lwpr: sensory data, feedback error in position and velocities, averaged errors in positions and velocities, normalized errors in positions and velocities        
    def ML_prediction(self, inputlwpr, fbackpos, fbackvel, meanerrp, meanerrv, normp, normv):
    #def ML_prediction(self, inputlwpr, fbackpos):
        ...
        #return self.output_x[0:2], self.output_x[2:4], self.output_DCN[0:2], self.output_DCN[2:4]#, self.model.init_D[0][0]

        return self.output_x[0:2], self.output_x[2:4], self.output_DCN[0:2], self.output_DCN[2:4]#, self.model.init_D[0][0]

    def ML_update(self, inputlwpr, train_LWPRoutput):  # what LWPR has to learn
