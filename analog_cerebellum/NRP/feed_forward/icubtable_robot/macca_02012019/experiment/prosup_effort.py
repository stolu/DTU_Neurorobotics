...
@nrp.MapRobotSubscriber("prosup_sub", Topic('/effort_command', std_msgs.msg.Float64MultiArray ))
...
@nrp.Robot2Neuron()
def prosup_effort (t, prosup_sub, joints_idx, proxy, duration):
    try:
        if t >3.:
            proxy.value.call( joints_idx.value[0] , prosup_sub.value.data[0],  None, duration.value)
            #clientLogger.info("prosup_sub "+str(prosup_sub.value.data[0]) )
    except Exception as e:
        clientLogger.info(" --> effort prosup Exception: "+str(e))
