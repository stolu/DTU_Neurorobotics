...
@nrp.MapRobotSubscriber("yaw_sub", Topic('/effort_command', std_msgs.msg.Float64MultiArray ))

# variable related to the effort service
@nrp.MapVariable("proxy_yaw",               initial_value = service_proxy)
@nrp.MapVariable("duration",            initial_value = wrench_dt)

@nrp.Robot2Neuron()
def yaw_effort (t, yaw_sub, joints_idx, proxy_yaw, duration):
    try:
        if t >3.:
            proxy_yaw.value.call( joints_idx.value[1] , yaw_sub.value.data[1],  None, duration.value)
     
    except Exception as e:
        clientLogger.info(" --> effort yaw Exception: "+str(e))
