...
# Red detection
lower_np = np.array([0, 50, 50], dtype = "uint8")
upper_np = np.array([10, 255, 255], dtype = "uint8")
zero     = np.array([  170, 50, 50], dtype = "uint8")#.array([  0, lower_np[1], lower_np[2]], dtype = "uint8")
end      = np.array([180, 255, 255], dtype = "uint8")#np.array([179, upper_np[1], upper_np[2]], dtype = "uint8")
...
def vision (t, starting_time, vision_debug,
    ...
    try:
        ...
        # Red detection
        mask1   = cv2.inRange(hsv_im, image_lower_bound.value, image_upper_bound.value)
        mask2   = cv2.inRange(hsv_im, image_zero.value, image_end.value)
        ...
        for c in cnts:
            ...
            cX = int(M["m10"] / M["m00"])
            cY = int(M["m01"] / M["m00"])
            cv2.circle(img_in, (cX, cY), 7, (255, 255, 255), -1)
            cv2.putText(img_in, "center", (cX - 20, cY - 20),  cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
