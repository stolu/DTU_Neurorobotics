...
class MLandC:
    ...
    def __init__(self, nin, njoints):
        ...
        for i in range(njoints):
            ...
            self.model[i].init_D        = 0.002 * np.eye(nin)         # 0.0002 (best PC)  #CRAWL 0.00001 # crawl solo 0.0001
            self.model[i].init_alpha    = 500 * np.ones([nin, nin])
            self.model[i].w_gen         = 0.5  # 0.1 default best
    ...
    def ML_prediction(self, inputlwpr, fbacktorq):      # inputlwpr array of inputs
        ...
        return self.output_ml, self.output_C, self.output_DCN, self.weights_mod
