...
sys.path.append("/users/stolu/workspace/DTU_neurorobotics/projects/lwpr_fable_fforward/scripts/fforward_macFableNew")
...
class Trajectory2j_circle(object):
    ...
    def calc_trajectory(self):
        for i in range(self.p):
            ...
            x[i] = fab.l1 * math.cos(q1[i]) + fab.l2 * math.cos((q1[i] + q2[i]))
            y[i] = fab.l1 * math.sin(q1[i]) + fab.l2 * math.sin((q1[i] + q2[i]))
...
#
    ...
    (torqLWPR[:, j], Ctorques[:, j]) = mlcj.ML_prediction(np.array([q1[j], q2[j], q1d[j], q2d[j], posr[0, j], posr[1, j]]), 
                                                            torquesLF[:, j])
#    if Ctorques[0, j] == "Nan":
#        Ctorques[0, j] = 0
#    if Ctorques[1, j] == "Nan":
