...
from lwpr import *
from AFEL_LWPRandC_class_velocity import *
from Robot import *
...
teta1 = [0 for k in range(n_iter + 1)]
teta2 = [0 for k in range(n_iter + 1)]
...
class Trajectory2j_circle(object):
    ...
    def calc_trajectory(self):
        for i in range(self.p):
            # Joint coordinates
            q1dd[i] = self.A1 * math.sin(2 * math.pi * self.t0)
            q1d[i] = ((-1/2)*math.pi) * self.A1 * math.cos(2 * math.pi * self.t0) #+ c
            #q1d[i] = -q1d[i]
            q1[i] = (-math.pow((1/2)*math.pi,2)) * self.A1 * math.sin(2 * math.pi * self.t0) #+ c
            #q1[i] = -q1[i]
            q2dd[i] = self.A2 * math.sin(2 * math.pi * self.t0 + self.phase)
            q2d[i] = ((-1/2)*math.pi) * self.A2 * math.cos(2 * math.pi * self.t0 + self.phase) #+ c
            #q2d[i] = -q2d[i]
            q2[i] = (-math.pow((1/2)*math.pi,2)) * self.A2 * math.sin(2 * math.pi * self.t0 + self.phase) #+ c
            #q2[i] = -q2[i]
            self.t0 += self.dt
            #Cartesian coordinates
            x[i] = fab.l1 * math.cos(q1[i]*math.pi/180) + fab.l2 * math.cos((q1[i] + q2[i])*math.pi/180)
            y[i] = fab.l1 * math.sin(q1[i]*math.pi/180) + fab.l2 * math.sin((q1[i] + q2[i])*math.pi/180)
        return q1, q2, q1d, q2d, q1dd, q2dd, x, y

T_circle = Trajectory2j_circle()
...
#
    ...
    (torqLWPR[:, j], Ctorques[:, j]) = mlcj.ML_prediction(np.array([q1[j], q2[j], q1d[j], q2d[j], posr[0, j], posr[1, j], velr[0,j], velr[1,j]]), torquesLF[:, j])
#    if Ctorques[0, j] == "Nan":
#        Ctorques[0, j] = 0
#    if Ctorques[1, j] == "Nan":
#       Ctorques[1, j] = 0
    ...
    mlcj.ML_update(np.array([q1[j], q2[j], q1d[j], q2d[j], posr[0, j], posr[1, j], velr[0,j], velr[1,j]]), (torquestot[:, j]))
    mlcj.ML_rfs()
    end_time = time.time()
    # plt.plot(torqLWPR)
...
scipy.io.savemat('TestForward_velocity_Mac080416.mat', dict(x=x, y=y, q1=q1, q2=q2, q1d=q1d, q2d=q2d, posr1=posr[0], posr2=posr[1], velr1 = velr[0], velr2 = velr[1], errp1=errp[0], errp2=errp[1], torquesLF1=torquesLF[0], torquesLF2=torquesLF[1], torqLWPR1=torqLWPR[0], torqLWPR2=torqLWPR[1], Ctorques1=Ctorques[0], Ctorques2=Ctorques[1], torquestot1=torquestot[0], torquestot2=torquestot[1]))
...
plt.figure(3)
plt.subplot(2, 1, 1)
plt.plot(m, q1d, label="Desired velocity joint 1")
plt.plot(m, velr[0], label="Real velocity joint 1")
plt.subplot(2, 1, 2)
plt.plot(m, q2d, label="Desired velocity joint 2")
plt.plot(m, velr[1], label="Real velocity joint 2")
plt.legend(bbox_to_anchor=(0., 1.02, 1., .102), loc=3,
           ncol=2, mode="expand", borderaxespad=0.)
plt.show()
