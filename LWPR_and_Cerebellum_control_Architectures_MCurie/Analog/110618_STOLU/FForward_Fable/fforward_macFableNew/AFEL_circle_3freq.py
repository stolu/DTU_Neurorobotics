...
class Trajectory2j_circle(object):
    ...
    def __init__(self, A, f0, k, w=2*math.pi, samplingfreq=0.01):
        self.A = A
        self.phase = phase
        self.t0 = t0
        self.p = 20002 #n_iter
    ...
    def __call__(self):
        for i in range(0, 1001):
            ...
            #derivate 2 pi (f+k x) cos(pi x (2 f+k x))
            
            q1[i] = self.A * math.sin(self.w*(self.f0*self.t0 + self.k/2*self.t0*self.t0))
            q1d[i] = self.w * (self.f0 + self.k * self.t0) * self.A * math.cos(self.w*(self.f0*self.t0 + self.k/2*self.t0*self.t0))
            q1dd[i] = -math.pow((self.w * (self.f0 + self.k * self.t0)),2) * self.A * math.sin(self.w*(self.f0*self.t0 + self.k/2*self.t0*self.t0))
            
            q2[i] = self.A * math.sin(self.w*(self.f0*self.t0 + self.k/2*self.t0*self.t0) + self.phase)
            q2d[i] = self.w * (self.f0 + self.k * self.t0) * self.A * math.cos(self.w*(self.f0*self.t0 + self.k/2*self.t0*self.t0) + self.phase)
            q2dd[i] = -math.pow((self.w * (self.f0 + self.k * self.t0)),2) * self.A * math.sin(self.w*(self.f0*self.t0 + self.k/2*self.t0*self.t0) + self.phase)
            self.t0 += self.sf
        for i in range(0,1001):
            j = i + 1000
            h = 1000 - i
...
T_circle = Trajectory2j_circle(20, 0.5, 0.1, 2*math.pi, 0.01)
...
scipy.io.savemat('TestForward_06_05_2016_Mac_3freq.mat', dict(x=x, y=y, q1=q1, q2=q2, q1d=q1d, q2d=q2d, posr1=posr[0], posr2=posr[1], velr1 = velr[0], velr2 = velr[1], errp1=errp[0], errp2=errp[1], torquesLF1=torquesLF[0], torquesLF2=torquesLF[1], torqLWPR1=torqLWPR[0], torqLWPR2=torqLWPR[1], Ctorques1=Ctorques[0], Ctorques2=Ctorques[1], torquestot1=torquestot[0], torquestot2=torquestot[1]))
