...
class CerebTestTune:
    ...
    def __init__(self, TEST, RESULTS_PATH, MODULES, JOINTS, MODEL=default_model):
        self.test = TEST
        self.test_PID = "PID_200"#"54", 200s simulation, 44" is 100s simulation, 28" is 60s simulation, "26" is 30s simulation
        self.test_recurrent = "recurrent" 
        self.test_fforward = "fforward"
    ...
    def merge_threads(self):
        for module in range(self.modules):
            ...
            for joint in self.joints:
                ...
                for contri_cereb in ["_recurrent-", "_feedforward-"]: 
                    exec("populations_total"+contri_cereb[:-1]+"_"  + str(module) + "_" + joint + "={}")
                    for key in populations:
                        exec("populations_total"+ contri_cereb[:-1]+"_" +str(module)+"_" + joint + "[" + "\"" + key + joint + contri_cereb + "\"" + "] = []")
                    
                    key = "GR"
                    exec("populations_total"+ contri_cereb[:-1]+"_" +str(module)+"_" + joint + "[" + "\"" + key + contri_cereb + "\"" + "] = []")

                    #list of groups in main populations
                    exec("populations_groups"+ contri_cereb[:-1]+"_"  + str(module) + "_" + joint + "={}")

                    for key in populations:
                        if "MF" in key:
                            for i in ["_cur_q", "_des_q", "_des_torque", "_des_qd"]:
                                exec("populations_groups"+ contri_cereb[:-1]+"_" +str(module)+"_" + joint + "[" + "\"" + key + joint + i+contri_cereb + "\"" + "] = []")
                        else:
                            for i in ["_pos", "_neg"]:
                                exec("populations_groups"+ contri_cereb[:-1]+"_" +str(module)+"_" + joint + "[" + "\"" + key + joint + i+contri_cereb + "\"" + "] = []")
        ...
        pop_types = ["_total_recurrent_", "_groups_recurrent_","_total_feedforward_", "_groups_feedforward_"]
        ...
        for joint in self.joints:
            for cells in ["MF", "GR", "PC", "DCN", "IO"]:
                if self.all_contributions:
                    if cells == "MF":
                        contributions = ["", "_cur_q", "_des_q", "_des_torque", "_des_qd"]
                    elif cells == "GR":
                        contributions = [""]
    ...
    def plot_joint(self, path=None):
        if path is None:
            ...
            path_recurrent = os.path.join(self.results_path, self.test_recurrent)
            path_fforward = os.path.join(self.results_path, self.test_fforward)
        ...
        joint_data_recurrent, title_recurrent = self.read_datafile(self, os.path.join(path_recurrent, 'joint_info.csv'))
        joint_data_fforward, title_fforward = self.read_datafile(self, os.path.join(path_fforward, 'joint_info.csv'))
        ...
        plt.plot(joint_data_PID[:,0], joint_data_PID[:,1], 'k',label = title[1])
        plt.plot(joint_data_recurrent[:,0], joint_data_recurrent[:,1], 'g',label = title[1])
        plt.plot(joint_data_fforward[:,0], joint_data_fforward[:,1], 'y',label = title[1])
        ...
        plt.legend(['q1_Cerebellum_hybrid', 'q1_des', 'q1_PID', 'q1_Cerebellum_recurrent', 'q1_Cerebellum_fforward'],bbox_to_anchor=(1.20,1), loc="upper right")
        plt.grid(True)
        ...
        error_q1_recurrent = joint_data_recurrent[:,1] - joint_data_recurrent[:,7]
        cumsum_e_q1_recurrent, moving_aves_e_q1_recurrent = [0], []

        dummy = 0.0
        for idx in range(len(error_q1_recurrent)): # enumerate(self.module.epr[0], 1):
            dummy_N = 0.0
            if (idx>N) and (idx+N) < len(error_q1_recurrent):
                for n in range (0, N):
                    dummy_N += abs(error_q1_recurrent[idx-N+n])
                moving_aves_e_q1_recurrent.append(dummy_N / (1.*N))
            elif idx>len(error_q1_recurrent)-N:
                it = len(error_q1_recurrent)-idx-1
                for n in range(0,it):
                    dummy_N += abs(error_q1_recurrent[idx-it+n])
                moving_aves_e_q1_recurrent.append(dummy_N / (1.*it + 2.))
            else:
                dummy += abs(error_q1_recurrent[idx])
                moving_aves_e_q1_recurrent.append(dummy/(idx+1.))


        error_q1_fforward = joint_data_fforward[:,1] - joint_data_fforward[:,7]
        cumsum_e_q1_fforward, moving_aves_e_q1_fforward = [0], []

        dummy = 0.0
        for idx in range(len(error_q1_fforward)): # enumerate(self.module.epr[0], 1):
            dummy_N = 0.0
            if (idx>N) and (idx+N) < len(error_q1_fforward):
                for n in range (0, N):
                    dummy_N += abs(error_q1_fforward[idx-N+n])
                moving_aves_e_q1_fforward.append(dummy_N / (1.*N))
            elif idx>len(error_q1_fforward)-N:
                it = len(error_q1_fforward)-idx-1
                for n in range(0,it):
                    dummy_N += abs(error_q1_fforward[idx-it+n])
                moving_aves_e_q1_fforward.append(dummy_N / (1.*it + 2.))
            else:
                dummy += abs(error_q1_fforward[idx])
                moving_aves_e_q1_fforward.append(dummy/(idx+1.))


        plt.plot(joint_data[:,0], moving_aves_e_q1, 'b',label = title[13])
        plt.plot(joint_data_PID[:,0], moving_aves_e_q1_PID, 'k',label = title_PID[13])
        plt.plot(joint_data_recurrent[:,0], moving_aves_e_q1_recurrent, 'g',label = title_recurrent[13])
        plt.plot(joint_data_fforward[:,0], moving_aves_e_q1_fforward, 'y',label = title_fforward[13])
        ...
        plt.xlabel('time [s]', fontsize=fntsz, color='black')
        plt.legend(['e_q1_Cerebellum','e_q1_PID', 'e_q1_recurrent','e_q1_fforward' ], bbox_to_anchor=(1.20,1), loc="upper right")
        plt.grid(True)
        ...
        plt.plot(joint_data_PID[:,0], joint_data_PID[:,2], 'k',label = title[2])
        plt.plot(joint_data_recurrent[:,0], joint_data_recurrent[:,2], 'g',label = title[2])
        plt.plot(joint_data_fforward[:,0], joint_data_fforward[:,2], 'y',label = title[2])
        
        plt.ylim(y_range_limit)
        plt.legend(['q2_Cerebellum_hybrid', 'q2_des', 'q2_PID', 'q2_Cerebellum_recurrent', 'q2_Cerebellum_fforward'],bbox_to_anchor=(1.20,1), loc="upper right")
        plt.ylabel('position [rad]', fontsize=fntsz, color='black')
        plt.xlabel('time [s]', fontsize=fntsz, color='black')
        plt.grid(True)
        ...
        error_q2_recurrent = joint_data_recurrent[:,2] - joint_data_recurrent[:,8]
        cumsum_e_q2_recurrent, moving_aves_e_q2_recurrent = [0], []

        dummy = 0.0
        for idx in range(len(error_q2_recurrent)): # enumerate(self.module.epr[0], 1):
            dummy_N = 0.0
            if (idx>N) and (idx+N) < len(error_q2_recurrent):
                for n in range (0, N):
                    dummy_N += abs(error_q2_recurrent[idx-N+n])
                moving_aves_e_q2_recurrent.append(dummy_N / (1.*N))
            elif idx>len(error_q2_recurrent)-N:
                it = len(error_q2_recurrent)-idx-1
                for n in range(0,it):
                    dummy_N += abs(error_q2_recurrent[idx-it+n])
                moving_aves_e_q2_recurrent.append(dummy_N / (1.*it + 2.))
            else:
                dummy += abs(error_q2_recurrent[idx])
                moving_aves_e_q2_recurrent.append(dummy/(idx+1.))


        error_q2_fforward = joint_data_fforward[:,2] - joint_data_fforward[:,8]
        cumsum_e_q2_fforward, moving_aves_e_q2_fforward = [0], []

        dummy = 0.0
        for idx in range(len(error_q2_fforward)): # enumerate(self.module.epr[0], 1):
            dummy_N = 0.0
            if (idx>N) and (idx+N) < len(error_q2_fforward):
                for n in range (0, N):
                    dummy_N += abs(error_q2_fforward[idx-N+n])
                moving_aves_e_q2_fforward.append(dummy_N / (1.*N))
            elif idx>len(error_q2_fforward)-N:
                it = len(error_q2_fforward)-idx-1
                for n in range(0,it):
                    dummy_N += abs(error_q2_fforward[idx-it+n])
                moving_aves_e_q2_fforward.append(dummy_N / (1.*it + 2.))
            else:
                dummy += abs(error_q2_fforward[idx])
                moving_aves_e_q2_fforward.append(dummy/(idx+1.))


        plt.plot(joint_data[:,0], moving_aves_e_q2, 'b',label = title[14])
        plt.plot(joint_data_PID[:,0], moving_aves_e_q2_PID, 'k',label = title_PID[14])
        plt.plot(joint_data_recurrent[:,0], moving_aves_e_q2_recurrent, 'g',label = title_recurrent[14])
        plt.plot(joint_data_fforward[:,0], moving_aves_e_q2_fforward, 'y',label = title_fforward[14])
        ...
        plt.ylabel('position error [rad] ', fontsize=fntsz, color='black')
        plt.xlabel('time [s]', fontsize=fntsz, color='black')
        plt.legend(['e_q2_Cerebellum','e_q2_PID', 'e_q2_recurrent','e_q2_fforward' ], bbox_to_anchor=(1.20,1), loc="upper right")
        plt.grid(True)
        fig = fig+1 
        # ** xy-plane
