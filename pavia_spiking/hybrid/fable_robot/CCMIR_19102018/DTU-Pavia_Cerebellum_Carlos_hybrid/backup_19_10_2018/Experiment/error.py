...
#Common Teaching signal for Recurrent and Feedforward
...
#Recurrent
@nrp.MapSpikeSource("MF_0_0_cur_q_recurrent",  nrp.map_neurons(range(0, nrp.config.brain_root.MF_n/nrp.config.brain_root.n_inputs_MF), lambda i: nrp.brain.MF_0_0_cur_q_recurrent[i]),  nrp.ac_source)
@nrp.MapSpikeSource("MF_0_1_cur_q_recurrent",  nrp.map_neurons(range(0, nrp.config.brain_root.MF_n/nrp.config.brain_root.n_inputs_MF), lambda i: nrp.brain.MF_0_1_cur_q_recurrent[i]),  nrp.ac_source)

#Feedforward
@nrp.MapSpikeSource("MF_0_0_cur_q_feedforward",  nrp.map_neurons(range(0, nrp.config.brain_root.MF_n/nrp.config.brain_root.n_inputs_MF), lambda i: nrp.brain.MF_0_0_cur_q_feedforward[i]),  nrp.ac_source)
@nrp.MapSpikeSource("MF_0_1_cur_q_feedforward",  nrp.map_neurons(range(0, nrp.config.brain_root.MF_n/nrp.config.brain_root.n_inputs_MF), lambda i: nrp.brain.MF_0_1_cur_q_feedforward[i]),  nrp.ac_source)
...
def error(  t, debug, n_joints, joints_name, joints_current, links_state_current, 
			...
			error_qdd, error_qdd_pub, recorder, 
			IO_0_0_q_pos, IO_0_1_q_pos,
			IO_0_0_q_neg,  IO_0_1_q_neg,
			MF_0_0_cur_q_recurrent, MF_0_1_cur_q_recurrent,
			MF_0_0_cur_q_feedforward, MF_0_1_cur_q_feedforward,
			radial_func_q_r, radial_func_q_ff, previous_error, previous_errors, integral_time, 
			previous_pos, previous_vel, veloc, acceleration):
	#velocity_moving_avg_window_size, velocity_average_, velocity_queue_list_
	...
	try:
		...
		MF_joint_q_cur_recurrent  = [MF_0_0_cur_q_recurrent,  MF_0_1_cur_q_recurrent]
		MF_joint_q_cur_feedforward  = [MF_0_0_cur_q_feedforward,  MF_0_1_cur_q_feedforward]
		...
		for k in range(0, n_joints.value):
			...
			cur_q_mossy_r  = radial_func_q_r.value.function(joints_current.value.position[k])
			cur_q_mossy_ff  = radial_func_q_ff.value.function(joints_current.value.position[k])

			m = 0
			for neuron_MFqcur  in MF_joint_q_cur_recurrent[k]:
				neuron_MFqcur.amplitude  = cur_q_mossy_r[m] 
				m = m + 1
		
			m = 0
			for neuron_MFqcur  in MF_joint_q_cur_feedforward[k]:
				neuron_MFqcur.amplitude  = cur_q_mossy_ff[m] 
				m = m + 1	
