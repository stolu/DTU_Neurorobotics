...
import sys, time
global time
sys.path.append("/home/dtu-neurorobotics/Documents/NRP/Experiments/fable_manipulation_dtu_spiking_cereb_recurrent")
...
@nrp.MapSpikeSource("IO_0_0_q_pos",  nrp.map_neurons(range(0, nrp.config.brain_root.IO_n/2), lambda i: nrp.brain.IO_0_0_pos[i]), nrp.poisson, weight=0.1)#nrp.ac_source)
@nrp.MapSpikeSource("IO_0_0_q_neg",  nrp.map_neurons(range(0, nrp.config.brain_root.IO_n/2), lambda i: nrp.brain.IO_0_0_neg[i]), nrp.poisson, weight=0.1)#nrp.ac_source)

#@nrp.MapSpikeSource("IO_0_0_qd_pos", nrp.map_neurons(range(nrp.config.brain_root.IO_n/4, nrp.config.brain_root.IO_n/2), lambda i: nrp.brain.IO_0_0_pos[i]), nrp.poisson, weight=0.1)#nrp.ac_source)
#@nrp.MapSpikeSource("IO_0_0_qd_neg", nrp.map_neurons(range(nrp.config.brain_root.IO_n/4, nrp.config.brain_root.IO_n/2), lambda i: nrp.brain.IO_0_0_neg[i]), nrp.poisson, weight=0.1)#nrp.ac_source)

@nrp.MapSpikeSource("IO_0_1_q_pos",  nrp.map_neurons(range(0, nrp.config.brain_root.IO_n/2), lambda i: nrp.brain.IO_0_1_pos[i]),  nrp.poisson, weight=0.1)#nrp.ac_source)
@nrp.MapSpikeSource("IO_0_1_q_neg",  nrp.map_neurons(range(0, nrp.config.brain_root.IO_n/2), lambda i: nrp.brain.IO_0_1_neg[i]),  nrp.poisson, weight=0.1)#nrp.ac_source)

#@nrp.MapSpikeSource("IO_0_1_qd_pos", nrp.map_neurons(range(nrp.config.brain_root.IO_n/4, nrp.config.brain_root.IO_n/2), lambda i: nrp.brain.IO_0_1_pos[i]), nrp.poisson, weight=0.1)#nrp.ac_source)
#@nrp.MapSpikeSource("IO_0_1_qd_neg", nrp.map_neurons(range(nrp.config.brain_root.IO_n/4, nrp.config.brain_root.IO_n/2), lambda i: nrp.brain.IO_0_1_neg[i]), nrp.poisson, weight=0.1)#nrp.ac_source)


# **  MOSSY FIBERS INPUTS  **
# send the current joint values to the mossy
@nrp.MapSpikeSource("MF_0_0_cur_q_plus",  nrp.map_neurons(range(0, nrp.config.brain_root.MF_n/nrp.config.brain_root.n_inputs_MF), lambda i: nrp.brain.MF_0_0_cur_q_plus[i]),  nrp.ac_source)
@nrp.MapSpikeSource("MF_0_0_cur_q_minus",  nrp.map_neurons(range(0, nrp.config.brain_root.MF_n/nrp.config.brain_root.n_inputs_MF), lambda i: nrp.brain.MF_0_0_cur_q_minus[i]),  nrp.ac_source)
@nrp.MapSpikeSource("MF_0_1_cur_q_plus",  nrp.map_neurons(range(0, nrp.config.brain_root.MF_n/nrp.config.brain_root.n_inputs_MF), lambda i: nrp.brain.MF_0_1_cur_q_plus[i]),  nrp.ac_source)
@nrp.MapSpikeSource("MF_0_1_cur_q_minus",  nrp.map_neurons(range(0, nrp.config.brain_root.MF_n/nrp.config.brain_root.n_inputs_MF), lambda i: nrp.brain.MF_0_1_cur_q_minus[i]),  nrp.ac_source)
...
def error(  t, debug, n_joints, joints_name, joints_current, links_state_current, 
			desired_trajectory_ee, desired_joints_trajectory, desired_LF_torque,
			error_q, error_qd, error_q_pub, error_qd_pub,
			error_qdd, error_qdd_pub,
			recorder,
			IO_0_0_q_pos, IO_0_1_q_pos,
			IO_0_0_q_neg,  IO_0_1_q_neg,
			MF_0_0_cur_q_plus, MF_0_0_cur_q_minus, MF_0_1_cur_q_plus, MF_0_1_cur_q_minus,
			MF_0_0_des_torque, MF_0_1_des_torque,
			radial_func_q, radial_func_torque,
			previous_error, previous_errors, integral_time, previous_pos, previous_vel, veloc, acceleration):
	...
	try:
		...
		MF_joint_q_cur_plus  = [MF_0_0_cur_q_plus,  MF_0_1_cur_q_plus]
		MF_joint_q_cur_minus  = [MF_0_0_cur_q_minus,  MF_0_1_cur_q_minus]
		...
		for k in range(0, n_joints.value):
			...
			error_qd.value.data[k]   = desired_joints_trajectory.value.velocity[k] - veloc.value.data[k]  #(error_q.value.data[k] - previous_error.value.data[k])/delta_t#joints_current.value.velocity[k]
			...
			#ed_io = ((10.0 - 0.0)/(20.0 - 0.0))*(abs(error_qd.value.data[k]) -20.0) + 10.0
			...
			cur_q_plus_mossy  = radial_func_q.value.function(joints_current.value.position[k])
			cur_q_minus_mossy = radial_func_q.value.function(previous_pos.value.data[k])
			if t==0:
			...
			n = 0
			p = 0
			for neuron_MFqcur_plus  in MF_joint_q_cur_plus[k]:
				neuron_MFqcur_plus.amplitude  = cur_q_plus_mossy[m] 
				m = m + 1
		
			for neuron_MFqcur_minus  in MF_joint_q_cur_minus[k]:
				neuron_MFqcur_minus.amplitude  = cur_q_minus_mossy[n] 
				n = n + 1
