...
@nrp.MapSpikeSink("DCN_0_1_pos",nrp.map_neurons(range(0, nrp.config.brain_root.DCN_n/2),lambda i: nrp.brain.DCN_0_1_pos[i]), nrp.population_rate, tau_fall=170.0, tau_rise=120.0)
@nrp.MapSpikeSink("DCN_0_1_neg",nrp.map_neurons(range(0, nrp.config.brain_root.DCN_n/2),lambda i: nrp.brain.DCN_0_1_neg[i]), nrp.population_rate, tau_fall=170.0, tau_rise=120.0)
...
@nrp.MapCSVRecorder("command_recorder", filename="command.csv", headers=["time", "filtered_LF1", "filtered_LF2","tau_lf1","tau_lf2","dcn1","dcn2","control_input1","control_input2"])
# -----------------------------------------------------------------------#
# ** "Brain" comunication **
@nrp.Neuron2Robot()
def controller( t, debug, proxy, duration, fable_robot, 
				desired_joints_trajectory, n_joints, joints_name, joints_current,
				error_q, error_qd, error_qdd, DCNcommand_pos, DCNcommand_neg, DCNcommand_total, LF, LFcommand, 
				K_p, K_d, K_i, command_recorder, plan_pub, desired_LF_torque_pub, corrected_error_q,
				DCN_0_0_pos, DCN_0_1_pos, DCN_0_0_neg, DCN_0_1_neg, motor1, motor2, torque_moving_avg_window_size,torque_average_,torque_queue_list_ ):
	...
	try:
		...
		desired_LF_torque_pub.send_message(LFcommand.value)
		...
		for j in range(0, n_joints.value):
			...
			DCNcommand_total.value.data[j] = (DCNcommand_pos.value.data[j]+ DCNcommand_neg.value.data[j])/k_scale_output[j]
		...
		try:
			for j in range(0, n_joints.value):
				...
				corrected_error_q.value.data[j] = error_q.value.data[j] + DCNcommand_total.value.data[j]
				#plan_pub.send_message(True) #REMEMBER TO DOUBLE CHECK THIS
		...
		control_input = [LFcommand.value.data[0], LFcommand.value.data[1]]
                ...
                #Filter the control input to remove noise
                
                
                
                
                
                #Calculating moving average of joint position
                ...
                for j in range(0, n_joints.value):
                    ...
                    torque_average_.value[j] = torque_average_.value[j] + torque_frac - torque_queue_list_.value[j][0]
                    torque_queue_list_.value[j].pop(0)
                
                    #Assigning values to filtered_LF1
                    
                    filtered_LF.append(torque_average_.value[j])
	...
	# ** record ** --- t,"tau_inv","tau_lf","dcn","control_input"	
		command_recorder.record_entry(t, 
									filtered_LF[0], filtered_LF[1], 
									LFcommand.value.data[0], LFcommand.value.data[1], 
									DCNcommand_total.value.data[0], DCNcommand_total.value.data[1] ,
