...
def create_cerebellum():
   ...
   excit                                                     inhib        
	...
	if ip.PLAST1_sinexp:
		...
		for module in range(ip.n_modules):
			for joint in range(ip.n_joints):
				...
				sim.nest.SetDefaults("stdp_synapse_sinexp"+str(module)+"_"+str(joint)+"_pos",{"A_minus":   ip.GC_PC_p["A_minus"],   # double - Amplitude of weight change for depression
																"A_plus":    ip.GC_PC_p["A_plus"],   # double - Amplitude of weight change for facilitation 
																"Wmin":      ip.GC_PC_p["Wmin"],    # double - Minimal synaptic weight 
																"Wmax":      ip.GC_PC_p["Wmax"],    # double - Maximal synaptic weight
																"vt":        vt["vt_"+str(module)+"_"+str(joint)+"_pos"][0]})

				sim.nest.SetDefaults("stdp_synapse_sinexp"+str(module)+"_"+str(joint)+"_neg",{"A_minus":   ip.GC_PC_p["A_minus"],   # double - Amplitude of weight change for depression
																"A_plus":    ip.GC_PC_p["A_plus"],   # double - Amplitude of weight change for facilitation 
																"Wmin":      ip.GC_PC_p["Wmin"],    # double - Minimal synaptic weight 
																"Wmax":      ip.GC_PC_p["Wmax"],    # double - Maximal synaptic weight
				...
				GC_PC[str(module)+"_"+str(joint)+"_pos"] = {"model":    "stdp_synapse_sinexp"+str(module)+"_"+str(joint)+"_pos",
														"weight":   ip.GC_PC_p["weight"],
														"delay":    ip.GC_PC_p["delay"]}
	...
	PC_DCN = sim.StaticSynapse(**ip.PC_DCN_p)
	MF_DCN = sim.StaticSynapse(**ip.MF_DCN_p)
		...
		for joint in range(ip.n_joints):
			...
			for P in range(PC_n/2):
				...
				sim.Projection(presynaptic_population= PCi["PCi_"+str(module)+"_"+str(joint)+"_pos"], 
					...
					connector=PC_DCN_c, synapse_type=PC_DCN, receptor_type='inhibitory')
			...
			sim.Projection(presynaptic_population=MF["MF_"+str(module)+"_"+str(joint)], postsynaptic_population=DCN["DCN_"+str(module)+"_"+str(joint)+"_pos"], 
				connector=MF_DCN_c, synapse_type=MF_DCN, receptor_type='excitatory')
