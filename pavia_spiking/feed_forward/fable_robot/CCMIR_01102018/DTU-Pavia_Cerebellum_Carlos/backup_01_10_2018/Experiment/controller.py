...
def controller( t, debug, proxy, duration, fable_robot, 
	...
	for j in range(0, n_joints.value):
		tau_dummy = K_p.value[j]*error_q.value.data[j] + K_d.value[j]*(- error_qd.value.data[j]  )  + K_i.value[j]*error_qdd.value.data[j] 
	...
	try:
		...
		k_scale_output = [40.0, 55.0]#[110.0, 110.0] #k_scale_output = (max_dcn_rate x n_dcn/2)/max_torque = 115.0 #### 110.0 was taken as approximation of 115.0
