...
rbf_io_q_out_max          = 15.0#0.02
rbf_io_q_out_min          = 0.000005
rbf_io_q_min_input_range  = 0.#- np.pi
rbf_io_q_max_input_range  = 2.0#np.pi
rbf_io_qd_out_max         = 15.0#0.02
rbf_io_qd_out_min         = 0.000005
rbf_io_qd_min_input_range = 0.
rbf_io_qd_max_input_range = + 12.

#rbf_io_q_err_out_max                  = 0.015
#rbf_io_q_err_out_min                  = 0.000005
#rbf_io_q_err_min_input_range          = 0.#- np.pi
##rbf_io_q_err_max_input_range          = 2.#+ np.pi
#rbf_io_qd_err_out_max                 = 0.015
#rbf_io_qd_err_out_min                 = 0.000005
#rbf_io_qd_err_min_input_range         = 0.
#rbf_io_qd_err_max_input_range         = 12.

# initialization of the RBF object for IO (it does not matter which joint, this will be specified when calling the function)
rbf_io_q  = RBF(nrp.config.brain_root.IO_n/4,  rbf_io_q_min_input_range,  rbf_io_q_max_input_range,  rbf_io_q_out_min,  rbf_io_q_out_max)
rbf_io_qd = RBF(nrp.config.brain_root.IO_n/4,  rbf_io_qd_min_input_range,  rbf_io_qd_max_input_range,  rbf_io_qd_out_min,  rbf_io_qd_out_max) 
...
def error(  t, debug, n_joints, joints_name, joints_current, links_state_current, 
	...
	try:
		...
		MF_joint_q_curr  = [ MF_0_0_cur_q,  MF_0_1_cur_q ]
		...
		for k in range(0, n_joints.value):
			...
			#if error_qd.value.data[k] >= 0.:
			#	for neuron_IOqd_p in IO_input_qd_pos[k]:
			#			neuron_IOqd_p.rate = ed_io#[iod]
			#			iod = iod + 1

			#if error_qd.value.data[k] < 0.:
			#	for neuron_IOqd_n in IO_input_qd_neg[k]:
			#			neuron_IOqd_n.rate = 1.0*ed_io#[iod]
			#			iod = iod + 1
